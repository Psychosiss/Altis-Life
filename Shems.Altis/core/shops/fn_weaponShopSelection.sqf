...
{
	_iS = [_item,(call life_weapon_shop_array)] call fnc_index;
	if(_iS == -1) then 
	{
		_price = 0;
	}
	...
	{
		_price = ((call life_weapon_shop_array) select _iS) select 1;
	};
	...
	else
{
	...
	{
		_priceTag ctrlSetStructuredText parseText format ["<t size='0.8'>Price: <t color='#ff0000'>$%1</t><br/>You lack: <t color='#8cff9b'>$%2</t></t>",[(_price)] call life_fnc_numberText,[(_price - life_money)] call life_fnc_numberText];
	}
		else
	{
		_priceTag ctrlSetStructuredText parseText format ["<t size='0.8'>Price: <t color='#8cff9b'>$%1</t></t>",[(_price)] call life_fnc_numberText];
