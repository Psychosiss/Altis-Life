...
if(_robber distance _shop > 2) exitWith {hint "Tu dois être plus prêt pour le braquer!";};
...
{ 
	...
	[[1,format["ALARME! - Distributeur: %1 est entrain de se faire braquer!", _shop]],"life_fnc_broadcast",west,false] call life_fnc_MP;
};
...
if(_rip) then
{
	...
	{ 
		...
		_shop switchMove ""; 
		hint "Vous devez resté a 2 metres du distributeur, maintenant il est bloqué."; 
		5 cutText ["","PLAIN"]; 
		_rip = false; 
	};
	...
	life_use_atm = true;
	if!(alive _robber) exitWith {};
	[[getPlayerUID _robber,name _robber,"211A"],"life_fnc_wantedAdd",false,false] call life_fnc_MP;
};
sleep 300;
