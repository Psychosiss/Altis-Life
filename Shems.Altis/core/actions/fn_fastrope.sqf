...
zlt_fnc_tossRopes = 
{
	...
	sleep random 0.3;
	if ( count (_heli getvariable ["zlt_ropes",[]]) != 0 ) exitWith 
	{ 
		zlt_mutexAction = false; 
	};
	...
	_heli spawn 
	{
		...
		while 
		{
			alive _heli and count (_heli getvariable ["zlt_ropes", []]) != 0 and abs (speed _heli) < 30 
		} do {
			sleep 0.3;
		};
...
{
	_veh = vehicle player;
	_flag = (_veh != player) and 
	{
		(not zlt_mutexAction)
	} and {
		count (_veh getvariable ["zlt_ropes", []]) == 0
	} and { 
		(typeof _veh) in zlt_rope_helis 
	} and {
		alive player and alive _veh and (abs (speed _veh) < 30) 
	};
	_flag;
...
zlt_fnc_fastropeaiunits = 
{
	...
	[_heli, _grunits] spawn 
	{
		...
		{
			if (!alive _x or isplayer _x or vehicle _x != _heli) then 
			{
				_units = _units - [_x];
			}; 
		} foreach _units;
		...
		{ 
			...
			_x spawn zlt_fnc_fastropeUnit; 
		} foreach _units;
		waitUntil {sleep 0.5; 
		{ 
			(getPos _x select 2) < 1 } count _units == count _units; 
		};
		sleep 10;
...
zlt_fnc_fastropeUnit = 
{
	...
	while 
	{
		alive _unit and (getPos _unit select 2) > 1 and (abs (speed _heli)) < 10  and _zc > -24
	} do {
...
player addAction["Jeter les cordes", zlt_fnc_createropes, [], -1, false, false, '','[] call zlt_fnc_ropes_cond'];
player addAction["Couper les cordes", zlt_fnc_removeropes, [], -1, false, false, '','not zlt_mutexAction and count ((vehicle player) getvariable ["zlt_ropes", []]) != 0'];
player addAction["Descendre en rappel", zlt_fnc_fastrope, [], 15, false, false, '','not zlt_mutexAction and count ((vehicle player) getvariable ["zlt_ropes", []]) != 0 and player != driver vehicle player'];
