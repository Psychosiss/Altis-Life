switch playerSide do
{
	case west:
	{
		life_actions = life_actions + [player addAction["Menotter",life_fnc_copRestrain,cursorTarget,-1,false,false,"",' if (isPlayer cursorTarget) then { ((side cursorTarget != west) && alive cursorTarget && cursorTarget distance player < 5 && !(cursorTarget getVariable ["restrained",false]) && speed cursorTarget < 1) };']];
	...
	case independent:
	{
		...
		life_actions = life_actions + [player addAction["Menotter",life_fnc_medRestrain,cursorTarget,-1,false,false,"",' if (isPlayer cursorTarget) then { ((side cursorTarget != independent) && alive cursorTarget && cursorTarget distance player < 5 && !(cursorTarget getVariable ["restrained",false]) && speed cursorTarget < 1) };']];
		life_actions = life_actions + [player addAction["Démenotter",life_fnc_unrestrain,cursorTarget,0,false,false,"",' if (!isNull cursorTarget) then { if (isPlayer cursorTarget) then { (cursorTarget isKindOf "Man" && player distance cursorTarget < 3.5 && (cursorTarget getVariable ["restrained",false]) && !(cursorTarget getVariable ["Escorting",false]))};};']];
	};
};
...
life_actions = life_actions + [player addAction["Baton lumineux en main",life_fnc_chemlightUse,"green",-1,false,false,"",' isNil "life_chemlight" && "Chemlight_green" in (magazines player) && vehicle player == player ']];
life_actions = life_actions + [player addAction["Baton lumineux en main",life_fnc_chemlightUse,"blue",-1,false,false,"",' isNil "life_chemlight" && "Chemlight_blue" in (magazines player) && vehicle player == player ']];
life_actions = life_actions + [player addAction["Lacher baton lumineux",{if(isNil "life_chemlight") exitWith {};if(isNull life_chemlight) exitWith {};detach life_chemlight; life_chemlight = nil;},"",-1,false,false,"",'!isNil "life_chemlight" && !isNull life_chemlight && vehicle player == player ']];
life_actions = life_actions + [player addAction["Coup de pied",life_fnc_punchBallsAction,"",0,false,false,"",' !isNull cursorTarget && player distance cursorTarget < 5 && isPlayer cursorTarget && animationState cursorTarget in ["Incapacitated","amovpercmstpsnonwnondnon_amovpercmstpssurwnondnon"]']];