...
if(_val > life_atmmoney) exitWith {hint "Vous n'avez pas autant dans votre compte bancaire!"};
_tax = [_val] call life_fnc_taxRate;
if((_val + _tax) > life_atmmoney) exitWith {hint format["Vous n'avez pas assez d'argent dans votre compte en banque, pour transférer %1 € vous aurez besoin %2 € comme taxe fiscale.",_val,_tax]};
