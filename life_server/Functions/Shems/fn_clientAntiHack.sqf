...
private ["_cfg","_patches","_sel","_name","_patches","_cash","_atm","_msg","_donator","_admin"];

waitUntil {!isNull player && player == player};

SHEMS_hackDetected = compileFinal 
"
	if (isServer) exitWith {};
	disableUserInput true;
	SHEMS_ANTI_HACK = [player, _this select 0];
	publicVariableServer ""SHEMS_ANTI_HACK"";
	sleep 0.5;
	SHEMS_KICK_ME = true;
	publicVariableServer ""SHEMS_KICK_ME"";
	_endM = compile PreProcessFileLineNumbers ""\a3\functions_f\Misc\fn_endMission.sqf"";
	[""SHEMS_ANTI_HACK"", false, false] call _endM;
";
...
for "_i" from 0 to count (_binConfigPatches)-1 do 
{
	...
	if (isClass _patchEntry) then 
	{
		...
		{
			_patches pushBack (configName _patchEntry);
		};
	};
};

if (count _patches > 0) exitWith 
{
	[format ["***ANTI-HACK***: %1:%2:%3 %4", "IllegalAddonPatch", profileName, getPlayerUID player, _patches]] call SHEMS_hackDetected;
};
...
{
	[format ["***ANTI-HACK***: %1:%2:%3 (%4)", "DevconLikeExecutor", profileName, getPlayerUID player, configName _x]] call SHEMS_hackDetected;
};
} forEach _children;

{
	...
	{
		[format ["***ANTI-HACK***: %1:%2:%3 (%4)", "ModifiedDisplay", profileName, getPlayerUID player, _x select 0]] call SHEMS_hackDetected;
	};
} forEach 
[
...
[] spawn 
{
	...
	{
		sleep 2;
		if (!isNull (findDisplay 30)) then 
		{
			closeDialog 0;
			[format ["***ANTI-HACK***: %1:%2:%3", "MenuBasedHack:30", profileName, getPlayerUID player]] call SHEMS_hackDetected;
		};

		if (!isNull ((findDisplay 49) displayCtrl 0)) then 
		{
			closeDialog 0;
			[format ["***ANTI-HACK***: %1:%2:%3", "MenuBasedHack:49", profileName, getPlayerUID player]] call SHEMS_hackDetected;
		};

		if (!isNull ((findDisplay 64) displayCtrl 101)) then 
		{
			closeDialog 0;
			[format ["***ANTI-HACK***: %1:%2:%3", "MenuBasedHack:64", profileName, getPlayerUID player]] call SHEMS_hackDetected;
		};

		if (!isNull (findDisplay 125)) then 
		{
			closeDialog 0;
			[format ["***ANTI-HACK***: %1:%2:%3", "MenuBasedHack:125", profileName, getPlayerUID player]] call SHEMS_hackDetected;
		};
		...
		if (!isNull (findDisplay 148)) then 
		{
			...
			{
				closeDialog 0;
				[format ["***ANTI-HACK***: %1:%2:%3", "MenuBasedHack:148", profileName, getPlayerUID player]] call SHEMS_hackDetected;
			};
		};

		if (!isNull (findDisplay 162)) then 
		{
			closeDialog 0;
			[format ["***ANTI-HACK***: %1:%2:%3", "MenuBasedHack:162", profileName, getPlayerUID player]] call SHEMS_hackDetected;
		};

		if (!isNull (findDisplay 3030)) then 
		{
			closeDialog 0;
			[format ["***ANTI-HACK***: %1:%2:%3", "MenuBasedHack:3030", profileName, getPlayerUID player]] call SHEMS_hackDetected;
		};

		if (!isNull (uiNamespace getVariable "RscDisplayRemoteMissions")) then 
		{
			closeDialog 0;
			[format ["***ANTI-HACK***: %1:%2:%3", "MenuBasedHack:RscDisplayRemoteMissions", profileName, getPlayerUID player]] call BWG_hackDetected;
		};

		if (!isNull (uiNamespace getVariable "RscDisplayDebugPublic")) then 
		{
			closeDialog 0;
			[format ["***ANTI-HACK***: %1:%2:%3", "MenuBasedHack:RscDisplayDebugPublic", profileName, getPlayerUID player]] call SHEMS_hackDetected;
		};

		if (!isNull (uiNamespace getVariable "RscDisplayArsenal")) then 
		{
			closeDialog 0;
			[format ["***ANTI-HACK***: %1:%2:%3", "MenuBasedHack:RscDisplayArsenal", profileName, getPlayerUID player]] call SHEMS_hackDetected;
		};

		if (!isNull (uiNamespace getVariable "RscDisplayInsertMarker")) then 
		{
			sleep 0.6;
			_action1 = buttonAction 1;
			_action2 = buttonAction 2;
			_title = ctrlText 1001;
			if (_action1 != "" || _action2 != "" || _title != localize "$STR_A3_RscDisplayInsertMarker_Title") exitWith 
			{
				closeDialog 0;
				[format ["***ANTI-HACK***: %1:%2:%3", "MenuBasedHack:RscDisplayInsertMarker", profileName, getPlayerUID player]] call SHEMS_hackDetected;
			};
		};
	};
};
...
_commonPaths = 
[
	"used for hacking", "wuat\screen.sqf", "scripts\defaultmenu.sqf", "menu\initmenu.sqf", "scr\exec.sqf", "scripts\exec.sqf", "menu\exec.sqf",
	"wuat\exec.sqf", "crinkly\keymenu.sqf", "scripts\ajmenu.sqf", "startup.sqf", "wookie_wuat\startup.sqf", "Wookie_Beta\exec.sqf", "Wookie\exec.sqf",
	"Wookie\antiantihax.sqf", "Wookie_Beta\antiantihax.sqf", "@DevCon\DevCon.pbo", "addons\@DevCon\DevCon.pbo", "DevCon.pbo", "@DevCon\addons\DevCon.pbo",
	"ShadowyFaze\exec.sqf", "monkified.sqf", "\wuat\screen.sqf", "\scripts\defaultmenu.sqf", "\menu\initmenu.sqf", "\scr\exec.sqf", "\scripts\exec.sqf",
	"\menu\exec.sqf", "\wuat\exec.sqf", "\crinkly\keymenu.sqf", "\scripts\ajmenu.sqf", "\startup.sqf", "\wookie_wuat\startup.sqf", "\Wookie_Beta\exec.sqf",
	"\Wookie\exec.sqf", "\Wookie\antiantihax.sqf", "\Wookie_Beta\antiantihax.sqf", "\@DevCon\DevCon.pbo", "\addons\@DevCon\DevCon.pbo", "\DevCon.pbo",
	"\ShadowyFaze\exec.sqf", "\scripts\defaultmenu.sqf", "\menu\initmenu.sqf", "\scr\exec.sqf","crinkly\keymenu.sqf", "\menu\exec.sqf", "\wuat\exec.sqf",
	"\crinkly\keymenu.sqf", "\scripts\ajmenu.sqf", "\startup.sqf", "\exec.sqf", "\wookie_wuat\startup.sqf","Wookie_Beta\exec.sqf", "\Wookie\exec.sqf",
	"\Wookie\antiantihax.sqf", "\Wookie_Beta\antiantihax.sqf", "\@DevCon\DevCon.pbo", "\addons\@DevCon\DevCon.pbo", "\DevCon.pbo", "\shadowyFaze\exec.sqf",
	"\monkified.sqf"
];

{
	...
	if (_contents != "") then 
	{
		[format ["***ANTI-HACK***: %1:%2:%3", "HackScript", profileName, getPlayerUID player]] call SHEMS_hackDetected;
	};
} forEach _commonPaths;

[] spawn 
{
	private ["_sleep", "_badVariables"];
	_sleep = 60;
	_badVariables = 
	[
		...
		"W_O_O_K_I_E_FUD_Pro_RE","W_O_O_K_I_E_FUD_Car_RE","W_O_O_K_I_E_FUD_FuckUp_GunStore","W_O_O_K_I_E_FUD_M_E_N_U_initMenu",
		...
		"E_X_T_A_S_Y_Menu_LOOOOOOOOOL","FOXBYPASS","Dayz_GUI_R","dayz_originalPlayer","zeus_star","_infiSTAR","ppe","ppe2","willschabppe33",
		"j___m___e____i_masterbate_to_bum_rape___hovering","j___m___e____i_masterbate_to_bum_rape___BelieveIcanFly","JJJJ_MMMM___EEEEEEE_LLYYSSTTIICCC_SHIT_RE_OLD",
		"JJJJ_MMMM___EEEEEEE_INIT_MENU","JJJJ_MMMM___EEEEEEE_SPAWN_VEH","JJJJ_MMMM___EEEEEEE_SPAWN_WEAPON","JJJJ_MMMM___EEEEEEE_fly","JJJJ_MMMM___EEEEEEE_NoRecoil",
		"JJJJ_MMMM___EEEEEEE_CLIP","JJJJ_MMMM___EEEEEEE_Invisable","JJJJ_MMMM___EEEEEEE_GiantCock","JJJJ_MMMM___EEEEEEE_JetBus","JJJJ_MMMM___EEEEEEE_BoxQuad",
		...
		"JJJJ_MMMM___EEEEEEE_Cash_1mill_t","JJJJ_MMMM___EEEEEEE_Cash_5mill_t","JJJJ_MMMM___EEEEEEE_LLYYSSTTIICCC_SHIT_RE","JJJJ_MMMM___EEEEEEE_JesusMode_t",
		"JJJJ_MMMM___EEEEEEE_M_E_N_U_fill_All","JJJJ_MMMM___EEEEEEE_M_E_N_U_initMenu","JJJJ_MMMM___EEEEEEE_OPTIONS","JJJJ_MMMM___EEEEEEE_PLAYERLOL",
		"JxMxE_hide","JME_Keybinds","JME_has_yet_to_fuck_this_shit","JME_deleteC","JME_Tele","JME_ANAL_PLOW","JME_M_E_N_U_initMenu","JME_M_E_N_U_hax_toggled",
		"JxMxE_Veh_M","JxMxE_LifeCash500k","JME_KillCursor","JME_OPTIONS","JME_M_E_N_U_fill_TROLLmenu","ASSPLUNGE","POLICE_IN_HELICOPTA","JxMxE_EBRP",
		"JxMxEsp","JxMxE_GOD","JxMxE_Heal","JxMxE_TP","JxMxE_Infect","JxMxE_secret","Metallica_vehicleg0dv3_infiSTAR","JJJJ_MMMM___EEEEEEE_GAPER",
		"JJJJ_MMMM___EEEEEEE_SMOKExWEEDxEVERYDAY_BUM_RAPE","JJJJ_MMMM___EEEEEEE_M_E_N_U_fill_Target","E3p","Jesus_MODE","MissileStrike","AL_Liscenses","NoIllegal",
		"NoWeight","m0nkyaatp_sadksadxa","m0nkyaatp_RANDSTR","myvar23","player_adminlevel","TNK","I_like_turtles","BIGM",
		"Does_Tonic_Like_to_take_Turtle_penis_in_the_ass_LODESTARS","Does_Tonic_Like_to_take_Turtle_penis_in_the_ass_LODESTAR1","GMod",
		"No_No_No_Tonic_likes_A_Big_Fat_Sheep_Cock_Right_in_the_bum_G0d_Mode","Sload","T3le","life_no_injection","Tonic_has_a_gaping_vagina","my_anus_hurtz","aKFerm",
		"aKMMenu","aKTitans","aKLikeaG0d","riasgremory_G0d_Mode","aKCarG0d","riasgremory_Car_Jesus","aKE5p","riasgremory_isseilol","aKPMark","riasgremory_Noobs",
		"riasgremory_Bitches","riasgremory_Map_Markers","aKUnMmo","jenesuispasuncheateur_unamo","aKVoit","Loljesaispasquoiecriremdr","isseigremory","gremorysama",
		"aKTaCu","aKCardetroy","aKGetKey","aKKillcursor","aKNoEscort","aKEscort","aKtroll","aKTPall","aKUnrestrain","aKNoEscortMe","aKNoTaze","aKJailplayer",
		"aKLisense","riasgremory_titans_shit_reold","Tonic_merde","jaimepaslepoisin_HLEAL","TTTT_IIII___TTTTTTT_RAP_FR","TTTT_IIII___TTTTTTT_REPGA",
		"TTTT_IIII___TTTTTTT_REPGAs","Root_Main4","Root_Pistol4","Root_Rifle4","Root_Machinegun4","Root_Sniper4","Root_Launcher4","Root_Attachement4"
	];
	while {true} do 
	{
		{
			...
			if (!isNil "_var") exitWith 
			{
				...
				[format ["***ANTI-HACK***: %1:%2:%3 (%4)", "HackVariable", profileName, getPlayerUID player, _x]] call SHEMS_hackDetected;
			};
		} forEach _this;
		sleep _sleep;
	};
...
[] spawn 
{
	waitUntil {sleep 5; unitRecoilCoefficient player < 1} exitWith 
	{
		[format ["***ANTI-HACK***: %1:%2:%3", "NoRecoilHack", profileName, getPlayerUID player]] call SHEMS_hackDetected;
	};
};
...
[] spawn 
{
	...
	while {true} do 
	{
		waitUntil {life_cash != _cash || life_atmcash != _atm};
		sleep 2;
		if (life_random_variable) then 
		{
			_cash = life_cash;
			_atm = life_atmcash;
			life_random_variable = false;
			[[getPlayerUid player, playerSide, life_cash, life_atmcash], "life_fnc_updateMoney", false, false] spawn BIS_fnc_MP;
		} else {
			titleText ["Vous avez été pris pour une injection d'argent. Profitez de votre ban! Shems.", "BLACK OUT"];
			_msg = format ["***ANTI-HACK***: %1 (%2): Money Injection (Cash: %3|%4; ATM: %5|%6)", profileName, getPlayerUID player, _cash, life_cash, _atm, life_atmcash];
			life_cash = _cash;
			life_atmcash = _atm;
			[_msg] call SHEMS_hackDetected;
		};
	};
};
...
[] spawn 
{
	while {true} do 
	{
		_donator = life_donator;
		_admin = life_adminlevel;
		sleep 5;
		if (life_donator != _donator || life_adminlevel != _admin) then 
		{
			[format ["***ANTI-HACK***: %1:%2:%3", "AdminDonatorLevelChange", profileName, getPlayerUID player]] call SHEMS_hackDetected;
		};
	};
};
		...
		while {true} do 
		{
			...
			if ((_new select 0 == _old select 0) && (_new select 1 == _old select 1)) then 
			{
				if (_kick) then 
				{
					...
					_kick = true;
					hint "Vous allez être kické pour avoir été absent + de 5 minutes";
				};
		} else {
					_kick = false;
				};
